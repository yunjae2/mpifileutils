...
pfind_find_results_t * pfind_find(pfind_options_t * opt)
{
    ...
    char *tmp;
    asprintf(&tmp, "%"PRIu64, opt->size);
    mfu_pred_add(pred_head, MFU_PRED_SIZE, (void *)tmp);

    if (opt->name_pattern) {
	    mfu_pred_add(pred_head, MFU_PRED_NAME, opt->name_pattern);
    }
